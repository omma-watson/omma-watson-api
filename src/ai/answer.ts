...
export const answerQuestion = async (question: string) => {
  ...
  const promptTemplate = new PromptTemplate({
    ...
    응답 형식:
    badge: 추천도. 0~3의 number. 추천(0), 양호(1), 주의(2), 위험(3) 중 하나. 너무 보수적이거나 민감하게 반응하지 않기 위해 긍정적으로 추천해주세요. 추천을 하더라도, 상세 설명에서 고려해야 할 점들을 함께 써주세요. 술, 담배, 마약, 날고기, 바퀴벌레 등 먹을수 없는거는 무조건 위험이야. 케이크는 양호, 라면류는 주의, 마라탕도 주의야. [[두부, 시금치, 현미밥, 콩밥, 깻잎은 추천(0)!!!!]].
